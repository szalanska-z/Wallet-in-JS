...
const addTransationPanel = document.querySelector(".add-transaction-panel");
...
const cancelnBtn = document.querySelector(".cancel");
...
const showPanel = () => {
  addTransationPanel.style.display = "flex";
};

const closePanel = () => {
  addTransationPanel.style.display = "none";
  clearInputs();
};
...
const checkCategory = (transaction) => {
  switch (transaction) {
    ...
    case "[ - ] Jedzenie":
      ...
      break;
    case "[ - ] Kino":
      categoryIcon = `<i class="fas fa-file"></i>`;
      break;
  }
};

const countMoney = (money) => {
  const newMoney = money.reduce((a, b) => {
    return a + b;
  });
...
const changeStyleToLight = () => {
  ...
  root.style.setProperty("--border-color", "rgb(0, 0, 0, 0.2)");
  root.style.setProperty("--third-color", "rgb(224, 221, 221, 0.3)");
};

const changeStyleToDark = () => {
  root.style.setProperty("--first-color", "#14161f");
  root.style.setProperty("--second-color", "#f9f9f9");
  root.style.setProperty("--border-color", "rgb(255, 255, 255, 0.4)");
  root.style.setProperty("--third-color", "rgb(20, 22, 31, 0.6)");
};

addTransationBtn.addEventListener("click", showPanel);
cancelnBtn.addEventListener("click", closePanel);
